...
//                    context.freshProcessDialog("Please Wait", "commu finish...");
    ...
    private void recall(final int i, final RequestParams recallRequest) {
        if (i > 0) {
            Log.e("SHETTY I", i + " repeat");

            recallRequest.setRequestcode("checkTxnStatus");
            recallRequest.setUsername(req.getUsername());
            recallRequest.setSessionId(req.getSessionId());

            recallRequest.setImei(req.getImei());
            recallRequest.setImsi(req.getImsi());
            ...
            NetworkController.getInstance().sendRequest(recallRequest, new ResponseListener() {
                ...
                public void onResponseSuccess(Response<ResponseParams> response, SuccessCustomDialog d) {
                    ...
                    if (response.body() != null) {
                     ...
                     //   p1000CallBacks.successCallback(getFailJSON("Status Msg :" + response.body().getMsg(),100));
                    } else {
                        if (req.getRequestcode() != null) {
                            if (req.getRequestcode().equalsIgnoreCase("microCashWithdrawIcici") || req.getRequestcode().equalsIgnoreCase("microCashWithdraw") || (req.getRequestcode() != null && req.getRequestcode().equalsIgnoreCase("doCardPayment"))) {
    ...
    public void onStartPBOC() throws RemoteException {
        ...
        try {
         //   p1000CallBacks.progressCallback("Starting PBOC");
            //context.freshProcessDialog("Please Wait", "start pboc...");
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void onRequestAmount() {
       ...
       // p1000CallBacks.progressCallback("Setting Amount");
...
/*
                new EnterDialog(context).showEnterDialog("please enter amount!", new OnEnterListener() {
            @Override
    ...
    public void onFindingCard(int cardType, Intent data) {
      ...
      //  Log.d("SHANKY", "PBOC Finding Choose card");
      //  ba.freshProcessDialog("Please Wait", "Finding Card Please Wait...");
      //  p1000CallBacks.progressCallback("Finding Card Please Wait...");
        switch (cardType)
        {
            case CardType.MAG_CARD:
                ...
                //For No PIN Cards
                //5596010036011755=21052262840000000
                try {
   ...
   /* @Override
    ...
    public void callRevRequest(RequestParams req, String arpc, String data5513, final ResponseParams resp, String DE39) {
        ...
        if (this.req.getRequestcode().equalsIgnoreCase(Constants.MICRO_CASH_WITHDRAW))
        ...
        r.setImei(req.getImei());
        r.setImsi(req.getImsi());
