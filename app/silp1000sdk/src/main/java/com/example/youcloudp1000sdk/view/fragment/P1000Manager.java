...
/*
        if (StaticValues.getIsY2000()) {
            GlobalData.getInstance().init(getActivity);
        }
*/
    ...
    public void showProcessDialog(String title, String msg) {
        if (processdialog == null)
            processdialog = new ProcessDialog(getActivity, title, msg);
        else {
    ...
    public void getMainKeys(P1000Request p1000Request, P1000CallBacks p1000CallBacks) {
       ...
       // requestParams.setImei("352672078500352");
        //requestParams.setImei("3dc0c6f6b9429aa");
        requestParams.setImei(p1000Request.getImei());
       // requestParams.setImei("null");
       // requestParams.setImei("442000227364352");
      //  requestParams.setImsi("abcd");
        requestParams.setImsi(p1000Request.getImsi());
                ...
                if (response.body() != null) {
                    if (response.body().getMkskKeys() != null) {
                        if (response.body().isStatus()) {
                           ...
                           /*         Session.setString(getActivity, SessionConstants.trackTmk, "" + response.body().getMkskKeys());
                                    Session.setString(getActivity, SessionConstants.tpkTmk, "" + response.body().getMkskKeys());
                                    Session.setString(getActivity, Constants.INJECTKEY, "T");
*/
                            ...
                            setResult(false, "FAILED", 99, "Unable to fetch key", p1000CallBacks);

                          //  FailCustomDialog failCustomDialog = new FailCustomDialog(getActivity, "Fail", "" + response.body().getMsg(), "");
                          //  failCustomDialog.show();
                        }
                    } else {
                        setResult(false, "FAILED", 99, "Fail key is NULL", p1000CallBacks);

                        //  FailCustomDialog failCustomDialog = new FailCustomDialog(getActivity, "Fail", "Key Is NULL", "");
                       // failCustomDialog.show();
                    }
                } else {
                    setResult(false, "FAILED", 99, "Check Internet Connection:Null Responce", p1000CallBacks);
...
*/
    ...
    public boolean login() {
        try {
            ...
            return bRet;
            ////getMainKeys(p1000CallBacks);
          //  loadPinKey(p1000CallBacks);
        } catch (Exception e) {
            //Toast.makeText(getActivi, "Login Exception", Toast.LENGTH_LONG).show();
            e.printStackTrace();
            return false;
    ...
    public void loadPinKey(P1000Request p1000Request, P1000CallBacks p1000CallBacks) {
        try {
            ...
            //4018061086018185D2311226241

            if (iRet) {
                //Toast.makeText(getActivity, "Load PIN Key Success", Toast.LENGTH_LONG).show();
                // Toast.makeText(getActivity, "load pin Key Success!");
                ...
                //Toast.makeText(getActivity, "load pin Key error");
                // Toast.makeText(getActivity, "pin key:" + pin);
                //  Toast.makeText(getActivity, "pin kcv:" + pin_kcv);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void loadTDKey(P1000Request p1000Request, P1000CallBacks p1000CallBacks) {
        try {
            ...
            if (iRet) {
                Session.setString(getActivity, SessionConstants.isMainKeyInjected, "F");

                Toast.makeText(getActivity, "Load Key Finish", Toast.LENGTH_LONG).show();
                ...
                // Toast.makeText(getActivity, "td key:" + tdkey);
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private void startPayment(P1000Request p1000Request, P1000CallBacks p1000CallBacks, String flag) {
        try {
            ...
            if (p1000Request.getRequestCode().getCode().equalsIgnoreCase(Constants.SDK_ENQUIRY_ICICI)) {
                ...
                txType = PBOCOption.ONLINE_INQUIRY;
            } else if (p1000Request.getRequestCode().getCode().equalsIgnoreCase(Constants.SDK_WITHDRAW_ICICI)) {
    ...
    private void onlineTrans(RequestParams req, String flag) {
        ...
        if (Session.getString(getActivity, Constants.INJECTKEY).equalsIgnoreCase("T")) {
            //load working keys
            loadPinKey(P1000Request, p1000CallBacks);
        ...
        if (!GlobalData.getInstance().getLogin()) {
            ...
            try {
                ...
                binder.startTransfer(txType, in, new onlinePBOCListener(getActivity, getAmt, req,p1000CallBacks,this));
                //ServiceManager.getInstence().getPboc().startTransfer(PBOCOption.ONLINE_PAY, in, new onlinePBOCListener(this, getAmt, req));
    ...
    private void checkTrasctionStatus(final P1000Request p1000Request, final StatusCallBack statusCallBack) {
        try {
            ...
            } else if (p1000Request.getRequestCode().getCode() == Constants.SDK_WITHDRAW_ICICI) {
